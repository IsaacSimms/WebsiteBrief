...
Write-Host ''
Write-Host '[1/6] Setting PowerShell execution policy to RemoteSigned...'
...
# To activate manually in future terminals, run: .\.venv\Scripts\Activate.ps1
Write-Host '  --> Activating .venv for this session...'
. .\.venv\Scripts\Activate.ps1

# == 3. Python Dependencies == #
Write-Host ''
Write-Host '[3/6] Installing Python packages from requirements.txt...'
pip install -r requirements.txt --quiet
Write-Host '  --> Done.'

# == 4. Playwright Browser == #
# Playwright needs a real browser binary to automate. We install Chromium,
# the open-source base for Google Chrome.
Write-Host ''
Write-Host '[4/6] Installing Playwright browser (Chromium)...'
python -m playwright install chromium
Write-Host '  --> Done.'

# == 5. Output Directories == #
# -Force means do not fail if the directories already exist.
Write-Host ''
Write-Host '[5/6] Creating output directories...'
...
Write-Host ''
Write-Host '[6/6] Setting up .env file...'
